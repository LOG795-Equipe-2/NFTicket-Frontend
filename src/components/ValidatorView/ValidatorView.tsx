...
export default function ValidatorView() {
  ...
  const exitSession = () => {
    BouncerService.deleteBouncer(eventId || "", id || "").then(() =>
      navigate("/")
    );
  };
    ...
    <CssBox className="Validator">
      ...
      {validatorState === ValidatorState.SUCCESS && (
        <Stack spacing={5} direction="column" alignItems="center">
          ...
          />
          <Button
            onClick={validateTicketID}
            variant="contained"
            color="info"
            disabled={
              !ticketID || ticketValidationMessage === ValidatorState.FETCHING
            }
          >
            {ticketValidationMessage === ValidatorState.FETCHING ? (
          ...
          {ticketValidationMessage === ValidatorState.ERROR && (
            <Dialog
              open={true}
              onClose={() => setTicketValidationMessage(null)}
            >
              <DialogContent>
                <Stack direction="row" alignItems="center" spacing={1}>
                  <ErrorIcon color="error" />
                  <DialogContentText>
                    Le billet n'a pu être confirmé
                  </DialogContentText>
                </Stack>
              </DialogContent>
              <DialogActions>
                <Button onClick={() => setTicketValidationMessage(null)}>
                  Réessayer
                </Button>
              </DialogActions>
            </Dialog>
