...
> {
  ...
  updateTicketStyling(key: string, value: any) {
    const ticketStyling = this.props.event.tickets[this.state.currentTicketSelected].styling;
    switch(key) {
        ...
        if (value && value.length) {
          ...
          fr.onload = () => {
            if (!!fr.result) {
              ...
              this.props.updateTicketStyling(this.state.currentTicketSelected, ticketStyling);
            }
          };
  ...
  render() {
    return this.props.event && this.props.event.tickets && this.props.event.tickets.length > 0 ? (
      <div className="EventCreator__customizeTickets" key="section-1">
        ...
        <div className="EventCreator__customizeTickets__editor">
          ...
          <div className="EventCreator__customizeTickets__editor__fields">
            ...
            <div className="row">
              <TextField onChange={(e) => this.updateTicketStyling("primaryColor", e.target.value)} value={this.props.event.tickets[this.state.currentTicketSelected].styling.primaryColor} type="color" label="Couleur principale" />
              <TextField onChange={(e) => this.updateTicketStyling("secondaryColor", e.target.value)} value={this.props.event.tickets[this.state.currentTicketSelected].styling.secondaryColor} type="color" label="Couleur secondaire" />
              <TextField onChange={(e) => this.updateTicketStyling("backgroundColor", e.target.value)} value={this.props.event.tickets[this.state.currentTicketSelected].styling.backgroundColor} type="color" label="Couleur de l'arrière plan" />
            </div>
            <Typography variant="subtitle1">Arrière-plan</Typography>
            <TextField onChange={(e) => this.updateTicketStyling("backgroundImage", (e.target as any).files)} defaultValue={this.props.event.tickets[this.state.currentTicketSelected].styling.backgroundImage}  className="single-input" type="file" />
            <Typography variant="subtitle1">Utiliser une bordure</Typography>
            <Switch value={this.props.event.tickets[this.state.currentTicketSelected].styling.useBorder} onChange={(e) => this.updateTicketStyling("useBorder", e.target.checked)}/>
          </div>
